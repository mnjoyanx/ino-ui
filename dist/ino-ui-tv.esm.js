...
var GridView = /*#__PURE__*/memo(function (_ref) {
  ...
  useEffect(function () {
    var calculateDimensions = function calculateDimensions() {
      ...
      var availableWidth = containerWidth || containerRect.width;
      var availableHeight = containerHeight || containerRect.height;
      var calculatedRowItems = rowItemsCount;
      var calculatedRows = rowCount;
      var calculatedItemWidth = itemWidth;
      var calculatedItemHeight = itemHeight;
      // If dimensions are not provided, calculate them
      if (!itemWidth || !itemHeight) {
        if (rowItemsCount && rowCount) {
          // Calculate based on provided row counts
          calculatedItemWidth = (availableWidth - gap * (rowItemsCount - 1)) / rowItemsCount;
          calculatedItemHeight = (availableHeight - gap * (rowCount - 1)) / rowCount;
        } else {
          // Calculate based on container size and aspect ratio
          var totalItems = data.length;
          var containerAspect = availableWidth / availableHeight;
          calculatedRowItems = Math.ceil(Math.sqrt(totalItems * containerAspect / aspectRatio));
          calculatedRows = Math.ceil(totalItems / calculatedRowItems);
          calculatedItemWidth = (availableWidth - gap * (calculatedRowItems - 1)) / calculatedRowItems;
    ...
    calculateDimensions();
  }, [containerWidth, containerHeight, rowItemsCount, rowCount, itemWidth, itemHeight, gap, aspectRatio, data.length]);
  ...
  var getItemStyle = useCallback(function (index) {
    ...
    }, _ref2[direction === 'rtl' ? 'right' : 'left'] = hIndex * (dimensions.itemWidth + gap) + "px", _ref2;
  ...
  var renderItems = useCallback(function () {
    ...
    return items;
  }, [startRow, bufferStart, bufferEnd, dimensions.rowItems, dimensions.rows, itemsTotal, uniqueKey, getItemStyle, activeIndex, isActive, data, onMouseEnterItem, renderItem]);
  useEffect(function () {
